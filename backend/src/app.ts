...
import {
  ...
  SESSION_TTL,
  SOC_SESSION_TTL,
  type AppSessionData as SessionData,
  type SocSessionData,
} from './security-state';
...
const generalRateLimiter: MiddlewareHandler = async (c, next) => {
  const ip = getClientIP(c);
  const rec = await securityState.consumeWindowCounter('general', ip, GENERAL_WINDOW);
  if (rec.count > GENERAL_MAX)
...
const loginRateLimiter: MiddlewareHandler = async (c, next) => {
  const ip = getClientIP(c);
  const now = Date.now();
  const rec = await securityState.getLoginAttempt(ip);
...
const checkoutRateLimiter: MiddlewareHandler = async (c, next) => {
  const ip = getClientIP(c);
  const rec = await securityState.consumeWindowCounter('checkout', ip, CHECKOUT_WINDOW);
  if (rec.count > MAX_CHECKOUT) {
...
const comentariosRateLimiter: MiddlewareHandler = async (c, next) => {
  const ip = getClientIP(c);
  const rec = await securityState.consumeWindowCounter('comments', ip, 60_000);
  if (rec.count > 10)
...
const authenticate: MiddlewareHandler<{ Variables: Variables }> = async (c, next) => {
  ...
  const session = token ? await securityState.getAppSession(token) : null;
  if (!token || !session) {
  ...
  if (Date.now() - session.createdAt > SESSION_TTL) {
    await securityState.deleteAppSession(token);
  ...
  if (!user?.twoFactorEnabled) {
    ...
    await securityState.updateAppSession(token, { twoFactorVerified: true });
  }
...
const authenticateSoc: MiddlewareHandler<{ Variables: Variables }> = async (c, next) => {
  const token = c.req.header('authorization');
  const session = token ? await securityState.getSocSession(token) : null;
  if (!token || !session) {
    return c.json({ error: 'No autenticado en SOC' }, 401);
  }
  if (Date.now() - session.createdAt > SOC_SESSION_TTL) {
    await securityState.deleteSocSession(token);
...
app.post('/api/register', honeypotAuth, loginRateLimiter, zValidator('json', RegisterSchema), async (c) => {
  ...
  try {
    ...
    await securityState.createAppSession(token, {
      id: user.id,
...
app.post('/api/login', honeypotAuth, loginRateLimiter, zValidator('json', LoginSchema), async (c) => {
  ...
  async function recordFailed() {
    const attempt = await securityState.recordLoginFailure(ip, MAX_ATTEMPTS, BLOCK_DURATION);
  ...
  try {
    ...
    await securityState.clearLoginAttempt(ip);
    const token = crypto.randomBytes(32).toString('hex');
    await securityState.createAppSession(token, {
      id: user.id,
      username: user.username,
...
const socLoginHandler = async (c: any) => {
  ...
  try {
    ...
    const token = crypto.randomBytes(32).toString('hex');
    await securityState.createSocSession(token, { id: admin.id, username: admin.username });
...
app.post('/api/logout', async (c) => {
  const token = c.req.header('authorization');
  if (token) await securityState.deleteAnySession(token);
  return c.json({ message: 'Sesión cerrada' });
});

const socLogoutHandler = async (c: any) => {
  const token = c.req.header('authorization');
  if (token) await securityState.deleteSocSession(token);
...
app.post('/api/usuario/avatar', authenticate, async (c) => {
  ...
  try {
    ...
    const token = c.req.header('authorization') ?? '';
    if (token) await securityState.updateAppSession(token, { avatar: avatarUrl });
...
app.put('/api/usuario/password', authenticate, zValidator('json', CambiarPasswordSchema), async (c) => {
  try {
    ...
    await securityState.deleteOtherAppSessions(user.id, tokenActual);
...
const socStatsHandler = async (c: any) => {
  try {
    ...
    const activeSessions = await securityState.countActiveSocSessions();
...
async function initDB() {
  ...
  const alterQueries = [
    ...
    `ALTER TABLE productos ADD COLUMN IF NOT EXISTS cpu TEXT`,
    `ALTER TABLE productos ADD COLUMN IF NOT EXISTS gpu TEXT`,
...
app.post('/api/2fa/enable', authenticate, zValidator('json', TwoFactorSetupSchema), async (c) => {
  try {
    ...
    await securityState.updateAppSession(c.req.header('authorization')!, { twoFactorVerified: true });
